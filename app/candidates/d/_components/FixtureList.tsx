      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-4">
        ...
        <div className="flex flex-wrap gap-1.5 items-center">
          {RECENT_SEASONS.map((s) => (
            <Link
              key={s}
              href={`/candidates/d/fixtures?season=${s}`}
        ...
        <div>
          ...
          <div className="space-y-2">
            {completed.map((f) => {
              ...
              const resultColour = f.result_text === 'Won'
                ? 'text-emerald-700 bg-emerald-50 border-emerald-100'
                : f.result_text === 'Lost'
                  ? 'text-rose-700 bg-rose-50 border-rose-100'
                  : 'text-gray-600 bg-gray-50 border-gray-100'
              const rowClass = 'w-full flex items-center gap-4 p-4 bg-gray-50/50 rounded-xl border border-gray-100 hover:border-gray-200 hover:bg-white transition-all text-left group'
              const body = (
                <>
                  <div className="text-center min-w-[56px]">
                  ...
                  {f.result_text ? (
                    <div className={`text-xs font-medium px-2.5 py-1 rounded-lg border flex-shrink-0 ${resultColour}`}>
                      {f.result_text}
                    </div>
                  ) : (
              ...
              return hasScorecard ? (
                <Link key={f.id} href={`/candidates/d/fixtures/${f.id}`} className={`${rowClass} no-underline text-inherit`}>
                  {body}
                </Link>
              ) : (
                <button key={f.id} onClick={() => setSelected(f)} className={rowClass}>
                  {body}
                </button>
              )
            })}
