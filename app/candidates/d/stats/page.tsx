...
import LeaderboardTable, { type BatterRow, type BowlerRow, type FielderRow } from '../_components/LeaderboardTable'
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-1.5 items-center">
          {recentSeasons.map(s => (
            <Link
              key={s}
              href={`/candidates/d/stats?season=${s}`}
          ...
          <Link
            href={`/candidates/d/stats?season=${ALL_TIME}`}
            className={`px-4 min-h-[44px] inline-flex items-center justify-center rounded-lg text-xs font-semibold transition-colors no-underline ${
              isAllTime
                ? 'bg-emerald-700 text-white'
